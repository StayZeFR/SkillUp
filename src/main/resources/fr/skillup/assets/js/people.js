...
App.onLoad(async () => {
    ...
    await initTable(peopleFiltered, 0, max);
...
async function reset() {
    peopleFiltered = await Bridge.getAsync("PeopleController", "getPeople");
...
async function initTable(people, start, max) {
    const container = document.getElementById("container-personnel");
    container.innerHTML = "";

    const slice = people.slice(start, Math.min(start + max, people.length));

    // Fetch all person skills in parallel instead of N sequential blocking calls
    const skillsResults = await Promise.all(
        slice.map(person => Bridge.getAsync("PeopleController", "getPersonSkills", [parseInt(person["id"])]))
    );

    slice.forEach((person, i) => {
        person["skills"] = skillsResults[i];
...
async function save() {
    ...
    closeModal();
    await reset();
    await initTable(peopleFiltered, 0, 6);
    Toast.fire({
        icon: "success",
